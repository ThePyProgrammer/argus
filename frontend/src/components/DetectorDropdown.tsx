...
import { useDetectorStore, type DetectorBackend } from '../stores/detectorStore';
import { CapabilityBadge } from './CapabilityBadge';

// D-06: detector-dropdown option rows show these three capability keys (heterogeneous values)
const BADGE_KEYS: Array<keyof DetectorBackend['capabilities']> = [
  'framework',
  'license',
  'cpu_latency_hint_ms',
];

export function DetectorDropdown({ onSelect }: { onSelect: (name: string) => void }) {
  const backends = useDetectorStore((s) => s.backends);
  const activeBackend = useDetectorStore((s) => s.activeBackend);
  const activeDisplay = useDetectorStore((s) => s.activeDisplay);
