...
- **High confidence for method families, lower confidence for direct X2 transfer:** CTDE, cooperative MARL, domain randomization, actuator modeling, humanoid locomotion RL, and collision avoidance have strong literature support, but no source proves they solve a real X2 swarm out of the box [M1][M2][M3][M4][M5][M6][M7][M8].
...
| `Gautam1704/Agibot-X2-Humanoid` [A4] | X2 ROS 2 package with humanoid URDF assets; first-round inspection found URDF/XML/STL and launch/RViz/world files under the package | ROS 2, URDF, MuJoCo XML, STL, RViz/Gazebo-style files | Apache-2.0 | Best starting point if license metadata is trusted; still audit provenance |
...
| `liulhs/AGIBOT` [A6] | REST bridge for X2 motion-control integration and generated AIMDK references | Python, Flask, ROS 2, generated docs | No detected license | Useful for orchestration ideas; needs security and provenance review |
...
| OmniHand SDK [A9] | AGIBOT hand SDK and model assets | Model files, C++/Python SDK, CANFD control; ROS details not visible on verified page | Mulan PSL v2 | Useful if X2 hand matches or a proxy hand is acceptable |
...
```mermaid
flowchart TD
    A[Mission objectives and experiment script] --> B[Task allocation / planner]
...
**Caption:** Proposed control stack for an AGIBOT X2 swarm. The learned multi-agent policy lives above the balance-critical locomotion layer and below interpretable task allocation. This is grounded in CTDE/cooperative MARL and legged/humanoid sim-to-real evidence [M1][M2][M3][M4][M5][M8][M10].
...
| Communication policy | Learned communication plus no-communication fallback | Train with packet loss, latency, bandwidth limits, stale messages [M6][M11] |
...
| Communication overfit | High | Medium | Train with latency/dropout/bandwidth limits and no-comm fallback [M6][M11] |
...
All final citation URLs below were checked with WebFetch on 2026-04-30. One initially considered MRTA PDF link returned 404 and a publisher DOI page returned 403, so that source was not used in the final citation set. A boundary/disclaimer page was reachable but exposed only limited visible text through fetch; claims based on detailed disclaimer content were therefore avoided.
...
- [M5] A Review of Cooperative Multi-Agent Deep Reinforcement Learning, https://arxiv.org/abs/1908.03963
- [M6] Learning to Communicate with Deep Multi-Agent Reinforcement Learning, https://arxiv.org/abs/1605.06676
...
- [M11] Learning Multiagent Communication with Backpropagation / CommNet, https://arxiv.org/abs/1605.07736
